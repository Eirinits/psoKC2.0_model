...
def perform_MaBoSS_simulation(n_profile, fname, profile_name, list_drugs, path_model, outputs, save_file, previous):
    
    #path_fname=path_model+"_"+str(n_profile)
    path_fname=path_model+"_"+profile_name
    ...
    for drug_id in list_drugs:
        ...
        simulation=maboss.load(path_fname_drug+".bnd", path_fname_drug + ".cfg")
        result=simulation.run()
        nodes_traj=result.get_nodes_probtraj()[outputs]
        final_results=nodes_traj.tail(1).to_csv (header=0,index=1,sep=' ')[:-1]
        # add the number of the simulation to the line finally saved
        #os.system("echo '"+str(n_profile)+" "+profile_name+" "+str(drug_id)+" "+final_results+"' >> "+save_file)
        os.system("echo '"+profile_name+" "+str(drug_id)+" "+final_results+"' >> "+save_file)
...
if __name__ == "__main__":   
    ...
    if not all(elem in nodes for elem in outputs):
        print("Output nodes must be present in the model")
        sys.exit(1)
    ...
    if len(cases_common)==0:
        cases_common.append('WT')

    os.system("echo n_profile\tPatient_ID\tDrug_ID\tTime\t"+'\t'.join(outputs)+" > "+save_file)
    ...
    for i in range(len(cases_common)):
        ...
        fname=model+"_"+str(i)
        path_fname=path_model+"_"+patient_id
        ...
        if args.drugs is not None:
            ...
            for drug_id, drug_indiv_dict in drugs_dict.items():
                ...
                drug_bnd_content=bnd_content
                for target, strength in drug_indiv_dict.items():
                    value=round(strength,3)
                    if strength<0:#i.e inhibitor drugs
                        drug_bnd_content = re.sub(r"(Node +"+target+" *{ *logic *= *)([^;]+)", r"\1(\2) & !anti_"+target,drug_bnd_content)
                        drug_bnd_content += "\nNode anti_{}".format(target) + " { " + "logic = (anti_{});".format(target) + " rate_up = @logic ? 0 : 0; rate_down = @logic ? 0 : 0;}"
                        drug_cfg_content = re.sub(r"d_"+target+" *= *[0-9]*\.*[0-9]*;", r"d_"+target+"=1;",drug_cfg_content)
                        drug_cfg_content +=  "\nanti_"+target+".is_internal = TRUE;\n[anti_"+target+"].istate = "+str(-value)+"[1], "+str(1+value)+"[0];"
                    elif strength>0:#i.e activating drugs
                        drug_bnd_content = re.sub(r"(Node +"+target+" *{ *logic *= *)([^;]+)", r"\1(\2) | pro_"+target,drug_bnd_content)
                        drug_bnd_content += "\nNode pro_{}".format(target) + " { " + "logic = (pro_{});".format(target) + " rate_up = @logic ? 0 : 0; rate_down = @logic ? 0 : 0;}"
                        drug_cfg_content = re.sub(r"u_"+target+" *= *[0-9]*\.*[0-9]*;", r"u_"+target+"=1;",drug_cfg_content)
                        drug_cfg_content +=  "\npro_"+target+".is_internal = TRUE;\n[pro_"+target+"].istate = "+str(value)+"[1], "+str(1-value)+"[0];"
                drug_bnd_content = drug_bnd_content.replace("{", "{\n").replace("}", "}\n").replace(";", ";\n")
                        
                open(path_fname+"_drug"+str(drug_id)+".cfg","w").write(drug_cfg_content)
                open(path_fname+"_drug"+str(drug_id)+".bnd","w").write(drug_bnd_content)
        else:
            os.rename(path_fname+".bnd", path_fname+".bnd")
            os.rename(path_fname+".cfg", path_fname+".cfg")
        
        while len(previous)<i-(nbprocesses-1):
            time.sleep(1)
        print(str(i)+": "+patient_id)
        p = mp.Process(target = perform_MaBoSS_simulation, args=(i,fname,patient_id, list(drugs_dict), path_model, outputs, save_file, previous))
        p.start()
        processes.append(p)
